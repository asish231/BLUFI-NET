...
public class Utility {
     ...
     * Saves the data in the app-specific documents directory.
     *
     * @param context  application context used to resolve app-specific storage
     * @param header   first description string on top of the file
     * @param fileName name of the file
     * @param data     actual data to be saved
     */
    public static void saveData(Context context, List<String> header, String fileName, List<?> data) throws IOException {
        // Convert arrays to delimited strings
        String headerString = TextUtils.join("\t", header);
        String dataString = TextUtils.join("\t", data);

        File root = context.getExternalFilesDir(Environment.DIRECTORY_DOCUMENTS);
        if (root == null) {
            root = new File(context.getFilesDir(), "metrics");
        }
        if (!root.exists() && !root.mkdirs()) {
            throw new IOException("Unable to create metrics directory: " + root);
        }
