...
public class ConnectBLETask {
    ...
    public void updateInternetStatus(boolean isOnline) {
        ...
        BluetoothGattService service = mGatt.getService(Constants.ServiceUUID);
        ...
        String val = isOnline ? getId() : (getId() + ";;offline");
        desc.setValue(val.getBytes());
        boolean res = mGatt.writeDescriptor(desc);
        Log.d(TAG, "OUD: updateInternetStatus: writeDescriptor client with internet result: " + res);
    }
}
