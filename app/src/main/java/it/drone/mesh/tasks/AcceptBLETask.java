...
public class AcceptBLETask {
        ...
        mGattServerCallback = new BluetoothGattServerCallback() {
            ...
            public void onDescriptorWriteRequest(BluetoothDevice device, int requestId, BluetoothGattDescriptor descriptor, boolean preparedWrite, boolean responseNeeded, int offset, byte[] value) {
                ...
                if (descriptor.getUuid().equals(Constants.ClientOnline_Configuration_UUID)) { //abilitazione di notifiche per ricevere messaggi
                    ...
                    boolean isOffline = valueStr.contains(";;offline");
                    String clientId = isOffline ? valueStr.split(";;")[0] : valueStr;
                    int[] infoClient = Utility.getIdArrayByString(clientId);
                    if (isOffline) {
                        if (infoClient[0] == Integer.parseInt(getId())) {
                            mNode.clearClientInternet(infoClient[1]);
                        } else {
                            ServerNode server = mNode.getServer("" + infoClient[0]);
                            if (server != null) {
                                server.clearClientInternet(infoClient[1]);
                            }
                        }
                        boolean stillHasInternet = Utility.isDeviceOnline(context) || mNode.getClientByteInternet() != 0;
                        mNode.setHasInternet(stillHasInternet);
                    } else {
                        if (infoClient[0] == Integer.parseInt(getId())) {
                            //è un mio client
                    ...
                    for (String id : nearDeviceMap.keySet()) {
                        ...
                        if (dev != null) {
                            ConnectBLETask client = Utility.createBroadcastClientWithInternet(dev, valueStr, context);
                            client.startClient();
                        }
                    }
    ...
    public void updateInternetStatus(boolean isOnline) {
        if (mNode == null) return;
        boolean current = mNode.hasInternet();
        if (current == isOnline) return;
        
        mNode.setHasInternet(isOnline);
        String propValue = isOnline ? getId() : (getId() + ";;offline");
        
        for (String id : nearDeviceMap.keySet()) {
            BluetoothDevice dev = nearDeviceMap.get(id);
            if (dev != null) {
                ConnectBLETask client = Utility.createBroadcastClientWithInternet(dev, propValue, context);
                client.startClient();
            }
        }
    }
